...
RPC_COUNT=9												# the highest number of RPC-client (starting from 2)
# Gebnerate a settinfgs file for each RPC-client
for ((i=2; i<=$RPC_COUNT; i++)) ; do
	mkdir -p ${TRANSMISSION_HOME}/tr$i
        mkdir -p ${TRANSMISSION_HOME}/tr$i/torrents
        mkdir -p ${TRANSMISSION_HOME}/tr$i/resume
        cp  ${TRANSMISSION_HOME}/settings.json ${TRANSMISSION_HOME}/tr$i				# Copy the settings.js file from base to each RPC-client

	sed -i "s/rpc-port\": 9091/rpc-port\": 909$i/g" "${TRANSMISSION_HOME}/tr$i/settings.json"	#  Assign the correct RPC-port so we can control this RPC-client
	sed -i "s/\"peer-port\": $PEER_PORT,/\"peer-port\": 3909$i,/g" "${TRANSMISSION_HOME}/tr$i/settings.json" # Change the peer-port (received from PIA) to a dummy port; PIA can only forward one port
done
...
for ((i=2; i<=$RPC_COUNT; i++)) ; do
	exec su --preserve-environment ${RUN_AS} -s /bin/bash -c "/usr/bin/transmission-daemon -g ${TRANSMISSION_HOME}/tr$i --logfile ${TRANSMISSION_HOME}/tr$i/transmission.log" &
done

sleep 10s
exec su --preserve-environment ${RUN_AS} -s /bin/bash -c "/scripts/Distribute.sh NORMAL >  ${TRANSMISSION_HOME}/Distribute.log" &
#############################
##### TinyProxy Startup #####
#############################
find_proxy_conf() {
